...
# End-to-end R script: tables + figures to ./output/
# ==============================================================
...
# ---- 1) Paths & I/O helpers ----
panel_path <- "equity_thresholds_panel.csv"  # <-- change if needed
...
save_table <- function(obj, name, fmt=c("html","csv")){
  ensure_dirs()
  if("html" %in% fmt){
    # modelsummary handles models + data frames; wrap in try to avoid aborting
    try(modelsummary::msummary(obj, output = file.path("output","tables", paste0(name, ".html"))), silent = TRUE)
  }
  if("csv" %in% fmt){
    if(is.list(obj)){
      # Convert list of models to data.frame
      try({
        out <- modelsummary::modelsummary(obj, output = "data.frame")
        write.csv(out, file.path("output","tables", paste0(name, ".csv")), row.names = FALSE)
      }, silent = TRUE)
    } else if(is.data.frame(obj)){
      write.csv(obj, file.path("output","tables", paste0(name, ".csv")), row.names = FALSE)
    }
  }
}

save_figure <- function(gg, name, width=7, height=5, dpi=300){
  ensure_dirs()
  ggsave(file.path("output","figures", paste0(name, ".png")), gg, width=width, height=height, dpi=dpi)
}
...
min_obs_guard <- function(df, min_n = 50){
  nrow(df) >= min_n
}
...
threshold_search <- function(df, grid_g = seq(10,95,by=1), outcome = "lmmr",
  ...
  if(!min_obs_guard(d, 50)){
    return(list(gamma = NA_real_, prof = tibble(gamma=grid_g, SSR=NA_real_)))
  }
  ssr <- sapply(grid_g, function(g){
    d2 <- d %>% mutate(
      x_low = pmin(.data[[expvar]], g),
...
fd_me_region <- function(fit, region, x, med_anemia, med_adbr, data_ref, h=0.1){
  base_row <- tibble(
    ...
    who_region              = region,
    # FE reference values (any valid)
    country_code            = data_ref$country_code[1],
    year                    = data_ref$year[1]
  )
  p <- as.numeric(predict(fit, newdata = base_row))
  (p[2] - p[1]) / (2*h)  # derivative on log scale per 1% coverage
}

# ---- 3) Load data & basic transforms ----
dt <- data.table::fread(panel_path, na.strings = c("", "NA", "NaN")) |> as_tibble()
dt <- dt |> filter(year >= 2000, year <= 2022)

# Required variables
core_vars <- c("country_code","country_name","who_region","year",
               "maternal_mortality_ratio","institutional_birth_pct",
               "anemia_prev_pct","adolescent_birth_rate")
missing_core <- setdiff(core_vars, names(dt))
if(length(missing_core)){
  stop(paste("Missing required variables in data:", paste(missing_core, collapse=", ")))
}

# Outcome on log scale (guard zeros) & winsorize raw MMR for robustness
dt <- dt |> mutate(
...
print(tab1)
save_table(tab1, "table01_overview", fmt=c("csv"))

# ---- 5) TABLE 2: Data availability by WHO region ----
tab2 <- dt |>
  group_by(who_region) |>
  summarise(
  ...
  arrange(who_region)
print(tab2, n = Inf)
save_table(tab2, "table02_availability", fmt=c("csv"))
...
bin_plot <- dt |>
  ...
  labs(x = "Institutional birth coverage (5% bins)",
       y = "Median MMR",
       title = "Binned association: institutional births vs MMR by WHO region") +
  theme_minimal()
save_figure(bin_plot, "fig01_binned_by_region")

# ---- 7) TABLE 3: TWFE + spline (primary model) ----
need_main <- c("lmmr","institutional_birth_pct","anemia_prev_pct",
               "adolescent_birth_rate","country_code","year")
d_main <- complete_for(dt, need_main)
if(nrow(d_main) == 0) stop("No complete cases for primary model. Check missingness.")
...
save_table(list("TWFE + spline (log MMR)" = m_spline), "table03_twfe_spline", fmt=c("html","csv"))

# ---- 8) FIG 2: Exposure–response curve (spline) ----
grid <- tibble(
...
curve_plot <- ggplot(grid, aes(institutional_birth_pct, pred_mmr)) +
  geom_line() +
  labs(x = "Institutional birth coverage (%)",
       y = "Predicted MMR",
       title = "Exposure–response from TWFE + spline") +
  theme_minimal()
save_figure(curve_plot, "fig02_exposure_response")

# ---- 9) TABLE 4: Region interactions (linear) + marginal effects via finite differences ----
need_lin <- c("lmmr","institutional_birth_pct","anemia_prev_pct",
              "adolescent_birth_rate","country_code","year","who_region")
...
if(nrow(d_lin) > 0){
  ...
  save_table(list("TWFE + region interactions (linear)" = m_region_lin),
             "table04_region_interactions", fmt=c("html","csv"))
  ...
  for (r in regions){
    for (xc in rep_levels){
      me <- tryCatch(
        fd_me_region(m_region_lin, r, xc, med_anemia, med_adbr, d_lin),
        error = function(e) NA_real_
      )
      ME_list[[length(ME_list)+1]] <- tibble(
        who_region = r,
        at_coverage = xc,
        dydx_logMMR_per1pct = me
      )
    }
  }
  ...
  save_table(ME_out, "table04b_marginal_effects_finite_diff", fmt=c("csv"))
} else {
  message("Skipping region interactions: insufficient complete cases.")
}

# ---- 10) TABLES 5–6 + FIG 3–4: Thresholds (global + region) ----
need_thr <- c("lmmr","institutional_birth_pct","anemia_prev_pct",
              "adolescent_birth_rate","country_code","year")
...
thr_plot <- ssr_prof |>
  ...
  {if(!is.na(thr_hat)) geom_vline(xintercept = thr_hat, linetype = 2) else NULL} +
  labs(x = "Candidate threshold (institutional birth %)",
       y = "Sum of squared residuals (SSR)",
       title = "Global threshold search profile") +
  theme_minimal()
save_figure(thr_plot, "fig03_threshold_ssr")

# TABLE 5: Threshold model at global gamma*
if(!is.na(thr_hat)){
  dt_thr <- d_thr |>
  ...
  save_table(list("TWFE + threshold (log MMR)" = m_thr),
             "table05_threshold_model", fmt=c("html","csv"))
} else {
  message("No valid global threshold estimated — skipping Table 5.")
}

# TABLE 6 + FIG 4: Region-specific thresholds
region_thr <- dt |>
  group_by(who_region) |>
  group_modify(~{
    ...
    tibble(threshold_pct = tr$gamma,
           SSR_min = suppressWarnings(min(tr$prof$SSR, na.rm=TRUE)))
  }) |>
  ungroup()
save_table(region_thr, "table06_region_specific_thresholds", fmt=c("csv"))
...
if(nrow(region_thr) > 0 && any(!is.na(region_thr$threshold_pct))){
  cat_plot <- region_thr |>
    ...
    theme_minimal()
  save_figure(cat_plot, "fig04_region_thresholds")
} else {
  message("No region thresholds to plot.")
}

# ---- 11) TABLE 7: Sensitivity on raw MMR ----
need_raw <- c("maternal_mortality_ratio","institutional_birth_pct",
...
if(nrow(d_raw) > 0){
  ...
  save_table(list("TWFE + spline (raw MMR)" = m_raw),
             "table07_sensitivity_raw_mmr", fmt=c("html","csv"))
} else {
  message("Skipping Table 7 (raw MMR): insufficient complete cases.")
}

# ---- 12) TABLE 8: Sensitivity adding skilled birth attendance (SBA) ----
if("skilled_birth_attendance_pct" %in% names(dt)){
  ...
  if(nrow(d_sba) > 0){
    ...
    save_table(list("TWFE + spline + SBA (log MMR)" = m_sba),
               "table08_sensitivity_add_sba", fmt=c("html","csv"))
  } else {
    message("Skipping SBA sensitivity: insufficient complete cases.")
  }
} else {
  message("SBA column not found; skipping Table 8.")
}
...
png(file.path("output","figures","fig05_dag.png"), width=900, height=600, res=150)
plot(g)
dev.off()

cat('\nAll outputs written to ./output/tables/ and ./output/figures/\n')
